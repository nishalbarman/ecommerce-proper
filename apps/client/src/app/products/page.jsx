...
export default function ProductList() {
  ...
  const parseInitialFilters = () => {
    try {
      ...
      return {
        ...
        color: [],
        price: [minPrice ? parseInt(minPrice) : 0, maxPrice ? parseInt(maxPrice) : 200],
        rating: 0,
      };
    } catch {
  ...
  const updateURL = useCallback(() => {
    ...
    if (appliedFilters.category.length > 0) params.set("category", appliedFilters.category.join(","));
    if (appliedFilters.price[0] > 0) params.set("minPrice", appliedFilters.price);
    if (appliedFilters.price[1] < 200) params.set("maxPrice", appliedFilters.price[1]);
  ...
  const { data, error, isLoading } = useGetProductsQuery(
    {
      ...
      maxPrice: appliedFilters.price[1] < 200 ? appliedFilters.price[1] : undefined,
    },
    {
    ...
    return (
      <div className="bg-gray-50 min-h-[72vh] flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-sm border p-6 text-center max-w-md w-full">
          ...
          <p className="text-gray-500 mb-5">We couldn’t load the products right now.</p>
          <Link
            href="/"
    ...
    <div className="min-h-screen ">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center gap-2">
          <span className="inline-block h-3 w-1.5 rounded-full" style={{ background: BRAND.primary }} />
          <span className="text-sm font-semibold" style={{ color: BRAND.primary }}>
            Catalogue
          </span>
        </div>
        <h1 className="mt-2 text-2xl sm:text-3xl font-bold text-gray-900">Products</h1>
      ...
      <div className="container mx-auto px-4 pb-8">
        ...
        <div className="md:hidden flex flex-col gap-4 mb-6">
          <form onSubmit={handleSearchSubmit} className="w-full flex">
            ...
            <button
              type="submit"
              className="px-5 py-2 rounded-r-lg text-white"
              style={{ background: BRAND.primary }}
            >
              Search
            </button>
          ...
          <section className="flex-1">
            ...
            {data?.data?.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-sm border p-10 text-center">
                ...
                <p className="mt-4 text-lg text-gray-600">No products found matching your criteria</p>
              </div>
            ) : (
              <>
                    ...
                    {Array.from({ length: Math.min(5, data.totalPages) }, (_, i) => {
                      ...
                      return (
                        <button
                          ...
                          style={page === pageNum ? { background: BRAND.primary, borderColor: BRAND.primary } : {}}
                        >
                          {pageNum}
                        </button>
                      );
